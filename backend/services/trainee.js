...
const createEditTraineeAccount = (req, res, next) => {
		...
		if (!!req.body._id) {
			...
			UserModel.findByIdAndUpdate(
				...
				.catch((err) => {
					res.status(500).json({
						success: false,
						message: 'Unable to update Student Account',
					});
				});
		} else {
			// check if user already exists in the database
			UserModel.findOne({ email: req.body.email, type: 'TRAINEE' }).then((doc) => {
				if (!!doc) {
					res.json({
						success: false,
...
let UpdateAnswers = (req, res, next) => {
	...
	var customAnswer = req.body.customAnswer;
